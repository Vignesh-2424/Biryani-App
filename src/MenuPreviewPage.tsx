...
// REVIEW SECTION COMPONENT - Same as ReviewPage but with disabled submit button
const MenuPreviewSection: React.FC = () => {
  const navigate = useNavigate();
  
  // Default cart items (sample items with quantities)
  ...
  const subtotal = Object.entries(defaultCartItems).reduce((total, [name, quantity]) => {
  ...
  return (
    <div className="bg-white px-6 py-8">
      ...
      {/* Cart Items - Default items */}
      <div className="space-y-4 mb-6">
        {Object.entries(defaultCartItems).map(([name, quantity]) => (
      ...
      <div className="mb-8">
        ...
        <div className="space-y-3">
          <div className="flex justify-between">
            ...
            <span className="text-black">{defaultFormData.fullName}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600 font-medium">Phone Number:</span>
            <span className="text-black">{defaultFormData.phoneNumber}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600 font-medium">Nearest Branch:</span>
            <span className="text-black">{defaultFormData.branch}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600 font-medium">Delivery Time:</span>
            <span className="text-black">{defaultFormData.deliveryTime}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600 font-medium">Address:</span>
            <span className="text-black">{defaultFormData.address}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600 font-medium">Pincode:</span>
            <span className="text-black">{defaultFormData.pincode}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600 font-medium">Landmark:</span>
            <span className="text-black">{defaultFormData.landmark || '-'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-center">
        <button
          disabled
          className="bg-gray-400 text-gray-600 font-semibold py-4 px-16 rounded-full cursor-not-allowed opacity-50"
        >
          Submit
        </button>
      </div>
...
// MAIN MENU PREVIEW PAGE COMPONENT
